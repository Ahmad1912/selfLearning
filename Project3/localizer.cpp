...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
    ...
    vector<float> row (w, belief_per_cell);
    vector< vector <float> > newGrid (h, row);
  ...
  float blurring) 
{
    ...
    vector < vector <float> > newG(h, vector <float> (w, 0));
    for (int i = 0; i < h; i++){
        for (int j = 0; j < w; j++){
            float new_i = (i + dy + h) % h;
            float new_j = (j + dx + w) % w;
            newG[new_i][new_j] = beliefs[i][j];
        }
        
    }

  return blur(newG, blurring);
}
	...
	float p_miss) 
{
    ...
    for (int i = 0; i < h; i++){
        for (int j = 0; j < w; j++){
            ...
            float res = (p_hit * me + (1 - me) * p_miss);
            newG[i][j] = res;
        }
    }

	return normalize(newG);
}
